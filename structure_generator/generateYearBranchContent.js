...
export const yearBranchIndexContent = (
    ...
    const popularPostArray = (await simpleGetJsonRequest('http://backend.rgpvnotes.in:8000/api/v1/popular_post.php?program_name=${programName}&scheme_name=${schemeName}')).content;
    
    // Fetch the branch year data
    const getBranchYearData = await fetch(
        'http://backend.rgpvnotes.in:8000/api/v1/search.php?program_name=${programName}&scheme_name=${schemeName}&subject_branch=${branchName}&subject_year=${yearName}',
    )
